...
# 读取Book_info中的信息
book_info_json = pd.read_json('../../Stage1_1/Result/Book_info.json')
book_info_json.to_csv('../Dataset/book_info.csv', index=False)
book_info = pd.read_csv('../Dataset/book_info.csv')

# 读取book_score中的信息
book_score = pd.read_csv('../Dataset/book_score.csv')
# 将Time进行离散化分桶处理
# 按天数进行离散化
...
for id_ in book_score["Book"]:
    rate.append(book_info[str(id_)].iloc[3])
    be_reading.append(book_info[str(id_)].iloc[9])
    have_read.append(book_info[str(id_)].iloc[10])
    wanna_read.append(book_info[str(id_)].iloc[11])
...
del final_info["Tag"]
# 写入文件
final_info.to_csv("../Dataset/book_final_info.csv", index=False)
