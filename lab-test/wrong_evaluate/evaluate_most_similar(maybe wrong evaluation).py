...
for p in path:
    ...
    for i in range(len(test_data)):
        ...
        tmp = [
            rank1[i][target_item_id].item(),
            rank2[i][target_item_id].item(),
            rank3[i][target_item_id].item(),
            rank4[i][target_item_id].item(),
        ]
        index = sorted(range(len(tmp)), key=lambda k: tmp[k])
        text.append(test_data[i]["predict"][index[0]].strip('"'))
    with open("./result_sota_text.json", "w") as f:
...
with open("./sota_most_similar.json", "w") as f:
